                ...
                <div class="card-content collapse show">
                  <div class="card-body card-dashboard">
                    <table id="datatable" class="table table-striped table-bordered zero-configuration">
                      ...
                      <tbody>
                      @foreach($idea_category as $key => $row)
                        <tr>
                            <td>{{$key + 1}}</td>
                            <td>
                              <a href="/admin/idea-subcategory/{{$row->id}}">{{$row->category}}</a>
                            </td>
                            <td>
...
$('#add_new').click(function(){
  ...
  $('#modal-title').text('Add Idea Category');
  $(".text-danger").remove();
...
function Save(){
  ...
  if(action_type == 1){
    ...
    $.ajax({
      url : '/admin/update-idea-category',
      type: "POST",
      data: formData,
      contentType: false,
      processData: false,
...
function Edit(id){
  spinner_body.show();   
  $.ajax({
    url : '/admin/edit-idea-category/'+id,
    type: "GET",
    dataType: "JSON",
    success: function(data)
    {
...
function Delete(id,status){
    ...
    if (r == true) {
      spinner_body.show();   
      $.ajax({
        url : '/admin/delete-idea-category/'+id+'/'+status,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {

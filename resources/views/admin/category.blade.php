    ...
    <div class="content-wrapper">
      <div class="content-header row">
        <div class="content-header-left col-md-6 col-12 mb-2 breadcrumb-new">
          <h3 class="content-header-title mb-0 d-inline-block">{{$language[39][Auth::guard('admin')->user()->lang]}}</h3>
          <div class="row breadcrumbs-top d-inline-block">
            <div class="breadcrumb-wrapper col-12">
              <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="/admin/dashboard">Home</a>
                </li>
                <li class="breadcrumb-item"><a href="#">{{$language[40][Auth::guard('admin')->user()->lang]}}</a>
                </li>
              </ol>
            </div>
          </div>
        ...
        <div class="content-header-right col-md-6 col-12">
            ...
            <button id="add_new" class="float-md-right btn btn-danger round btn-glow px-2" type="button">{{$language[45][Auth::guard('admin')->user()->lang]}}</button>
            @endif
            ...
            <div class="col-12">
              <div class="card">
                <div class="card-header">
                  <h4 class="card-title">{{$language[40][Auth::guard('admin')->user()->lang]}}</h4>
                    ...
                    <table id="datatable" class="table table-striped table-bordered zero-configuration">
                      <thead>
                        <tr>
                            ...
                            <th>{{$language[43][Auth::guard('admin')->user()->lang]}}</th>
                            ...
                            <td>
                                <div class="btn-group mr-1 mb-1">
                                    ...
                                    <div class="dropdown-menu open-left arrow" x-placement="bottom-start" style="position: absolute; transform: translate3d(0px, 40px, 0px); top: 0px; left: 0px; will-change: transform;">
                                        @if($role_get->product_category_edit == 'on')
                                        <button onclick="Edit({{$row->id}})"class="dropdown-item" type="button">Edit</button>
                                        @endif 
                                        @if($role_get->product_category_delete == 'on')
...
function Delete(id,status){
    ...
    if (r == true) {
      spinner_body.show();   
      $.ajax({
        url : '/admin/delete-category/'+id+'/'+status,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {

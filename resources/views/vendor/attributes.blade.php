                    ...
                    <table id="datatable" class="table table-striped table-bordered zero-configuration">
                      ...
                      @foreach($attributes as $key => $row)
                        <tr>
                            <td>{{$key + 1}}</td>
                            <td>
                            <a href="/vendor/attribute-fields/{{$row->id}}">{{$row->attribute_name}}</a>
                            </td>
                            <td>
...
function Delete(id){
    ...
    if (r == true) {
      spinner_body.show();   
      $.ajax({
        url : '/vendor/delete-attributes/'+id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
